...
public interface TicketDao extends CrudRepository<Ticket, Integer> {
	...
	@Query("SELECT t FROM Ticket t  where t.user.userId= :userId and t.eventId= :eventId and t.isCanceled = true")
	Ticket getTicketbyUserAndEvent(@Param("userId") int userId, @Param("eventId") int eventId);

	@Query("SELECT t FROM Ticket t  where t.user.userId= :userId and t.eventId= :eventId and t.isCanceled = false")
	List<Ticket> getValidTicketbyUserAndEvent(@Param("userId") int userId, @Param("eventId") int eventId);
